...
public class MYRFinancingController {
    ...
    @ApiImplicitParam(name = "taskId", value = "任务id", required = true, dataType = "Integer", paramType = "path")
    public Result myrSalesmanSupplyInvestigationMaterialMethod(@PathVariable("taskId")int financeId) {
...
//            }
    ...
    @RequestMapping(value = "/riskmanager/need/investigation/material/{taskId}", method = RequestMethod.PUT)
    @ApiOperation(value = "风控人员确认需要补充尽调材料", notes = "风控人员确认需要补充尽调材料")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "taskId", value = "金融申请单id", required = true, dataType = "Integer", paramType = "path"),
            @ApiImplicitParam(name = "pass", value = "审核结果: 0:审核不通过, 1:审核通过", required = true, dataType = "Integer", paramType = "query")
    })
    public Result myrRiskManagerConfirmNeedSupplyInvestigationMaterialMethod(@PathVariable("taskId")String taskId,
                                                                             @RequestParam(value = "pass", required = true) int pass,
