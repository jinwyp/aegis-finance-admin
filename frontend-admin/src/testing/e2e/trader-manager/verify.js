/**
 * Created by gonghuihui on 16/11/30.
 */
...
describe('线上交易员管理员',function(){
    ...
    beforeAll(function(){
        ...
        loginUserName.sendKeys(user.adminTrader.username);
        loginPassword.sendKeys(user.adminTrader.password);
        loginButton.click();
...
describe('线上交易员管理员-待办详情',function(){
        ...
        it('分配线上交易员', function () {
            ...
            return element.all(by.css('.custom-select ul li')).filter(function (elem) {
                ...
                rows[0].getText().then(function (text) {
                    expect(text).toBe('线交易');
                });
                rows[0].click();
                expect(element(by.css('.custom-select label')).getText()).toContain('线交易');
                fpButton.click();
            });
        });
