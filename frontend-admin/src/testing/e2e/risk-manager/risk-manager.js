...
describe('风控员管理员',function(){
    ...
    beforeAll(function(){
        ...
        loginUserName.sendKeys(user.adminRiskmanager1.username);
        loginPassword.sendKeys(user.adminRiskmanager1.password);
        loginButton.click();
        util.waitForUrlToChangeTo('/finance/admin/home/tasks/pending');
    });

    describe('风控员管理员-我的待办',function(){
...
describe('风控员管理员-待办详情',function(){
    ...
    it('分配风控员', function () {
        ...
        return element.all(by.css('.custom-select ul li')).filter(function (elem) {
            ...
            rows[0].click();
            expect(element(by.css('.custom-select label')).getText()).toContain('风控');
            fpButton.click();
        });
    });
