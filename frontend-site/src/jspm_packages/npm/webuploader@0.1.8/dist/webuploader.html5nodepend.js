...
(function(root, factory) {
  ...
  define('dollar-builtin', [], function() {
    ...
    function each(obj, iterator) {
      ...
      if (typeof obj !== 'function' && typeof obj.length === 'number') {
        ...
        for (i in obj) {
          if (hasOwn.call(obj, i) && iterator.call(obj[i], i, obj[i]) === false) {
            return obj;
          }
        }
      }
    ...
    function $(elem) {
      var api = {};
      elem = typeof elem === 'string' && rId.test(elem) ? doc.getElementById(RegExp.$1) : elem;
      if (elem) {
        api[0] = elem;
        api.length = 1;
      }
  ...
  define('promise-builtin', ['dollar'], function($) {
    ...
    function Deferred(func) {
      var tuples = [['resolve', 'done', Callbacks(true), 'resolved'], ['reject', 'fail', Callbacks(true), 'rejected'], ['notify', 'progress', Callbacks()]],
          state = 'pending',
          promise = {
            state: function() {
              return state;
