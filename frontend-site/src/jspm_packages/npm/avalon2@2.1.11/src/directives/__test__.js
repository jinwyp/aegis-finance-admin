...
describe('attr', function() {
  it("test", function() {
    ...
    expect(body.changeAttr).to.eql({
      'ms-controller': false,
      'data-controller': 'test'
    });
    var firstDiv = body.children[0];
    expect(firstDiv.type).to.eql('div');
    expect(firstDiv.props).to.eql({
      'ms-attr-title': 'aaa',
      title: 11
    });
    expect(firstDiv.children[0].type).to.equal("b");
    expect(firstDiv.children[0].props).to.eql({
      'ms-attr-title': 'bbb',
      title: 22
    });
    ...
    expect(thirdDiv.props).to.eql({
      'ms-attr-title': 'ddd',
      title: 44
    });
    expect(thirdDiv.children[0].type).to.equal("b");
    expect(thirdDiv.children[0].props).to.eql({
      'ms-attr-title': 'eee',
      title: 55
    });
    ...
    expect(fourthDiv.children[0].props).to.eql({
      'ms-attr-title': 'fff',
      title: 66
    });
    vm.fff = 77;
...
describe('class', function() {
  it("test", function() {
    ...
    expect(div.children[1].classData).to.be.eql({
      aaa: true,
      bbb: true,
      ccc: true
    });
    expect(div.children[2].classData).to.be.eql({
      bbb: true,
      ccc: true,
      aaa: true,
      hot: true,
    ...
    expect(div.children[3].classData).to.be.eql({
      bbbdog: true,
      ccc: true,
      aaa: true
    });
    expect(div.children[4].classData).to.be.eql({
      xxx: true,
      yyy: true,
      zzz: true
    });
    expect(div.children[5].classData).to.be.eql({
      XXX: true,
      yYY: true,
      ZZZ: true
    });
