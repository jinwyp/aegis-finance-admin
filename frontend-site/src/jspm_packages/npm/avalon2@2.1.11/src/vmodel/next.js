...
function canObserve(key, value, skipArray) {
  return !skipArray[key] && (key.charAt(0) !== '$') && value && !value.$id && (typeof value === 'object') && (!value.nodeType && !value.nodeName);
}
...
if (avalon.window.Proxy) {
  ...
  function masterFactory(definition, heirloom, options) {
    ...
    for (var key in definition) {
      ...
      if (canObserve(key, val, $skipArray)) {
        definition[key] = $$midway.modelAdaptor(val, 0, heirloom, {id: definition.$id + '.' + key});
      }
    }
  ...
  function proxyfy(definition) {
    return Proxy.create ? Proxy.create(definition, handlers) : new Proxy(definition, handlers);
  }
  ...
  function slaveFactory(before, after, heirloom) {
    for (var key in after) {
      ...
      if (!before.hasOwnProperty(key)) {
        var val = after[key];
        if (canObserve(key, val, {})) {
          before[key] = $$midway.modelAdaptor(val, 0, heirloom, {id: before.$id + '.' + key});
        }
      }
    }
    if (key in before) {
...
var handlers = {
  deleteProperty: function(target, name) {
    if (target.hasOwnProperty(name)) {
      target.$track = (';;' + target.$track + ';;').replace(';;' + name + ';;', '').slice(2, -2);
    }
  },
